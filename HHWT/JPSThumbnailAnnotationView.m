...
- (void)setupImageView {
    ...
    _loadingView.center = CGPointMake(25, 23);
    _loadingView.hidesWhenStopped = NO;
    [_imageView addSubview:_loadingView];

    
    
//    _loadingView = [[UIActivityIndicatorView alloc] initWithFrame:CGRectMake(20, 20, 30, 30)];
//    [_imageView addSubview:_loadingView];
...
- (void)updateWithThumbnail:(JPSThumbnail *)thumbnail {
    ...
    [self.imageView sd_setImageWithURL:[NSURL URLWithString:thumbnail.imageURL] placeholderImage:[UIImage imageNamed:@""] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
        _loadingView.hidden = YES;
        [_loadingView stopAnimating];
    }];
    
    self.dataArray1 = thumbnail.dataArray;
...
- (void)expand {
    ...
    BOOL iOS7 = [[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0f;
    UIButtonType buttonType = iOS7 ? UIButtonTypeSystem : UIButtonTypeCustom;
    _btnPush = [UIButton buttonWithType:buttonType];
    _btnPush.tintColor = [UIColor clearColor];
    
    _btnPush.frame = CGRectMake(0,0,self.frame.size.width,self.frame.size.height);
    
    [_btnPush addTarget:self action:@selector(didTapDisclosureButton:) forControlEvents:UIControlEventTouchDown];
    [self addSubview:_btnPush];
...
- (void)didTapDisclosureButton:(NSString *)str
{
    if (self.state == JPSThumbnailAnnotationViewStateExpanded) {
    if(self.dataArray1.count > 0){
        NSDictionary *userInfo = [self.dataArray1 objectAtIndex:0];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"ArrowFire" object:nil userInfo:userInfo];
    }
